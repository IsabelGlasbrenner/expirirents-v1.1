...
class Listings extends React.Component {
	...
	filterData(data, filters) {
		...
		filtData = filtData.filter(function (filtData) {
			...
			return Date.parse(filtData.availableDate.DateSpan[0].startDate) <= Date.parse(filters.to) && Date.parse(filtData.availableDate.DateSpan[0].endDate) >= Date.parse(filters.from);
		})
		return filtData;
	}
	...
	handleSearch() {
		this.setState({fData: this.filterData(this.state.data,{type: this.state.type, location: this.state.location, to: this.state.to, from: this.state.from})});
	}
